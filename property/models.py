...
class Complaint(models.Model):
    author = models.ForeignKey(User, null=True, on_delete=models.SET_NULL, related_name='complaints', verbose_name='Кто жаловался')
    ...
    def __str__(self):
        return f'Жалоба от {self.author}'

class Owner(models.Model):
    full_name = models.CharField("ФИО владельца", max_length=200, db_index=True)
    owners_phonenumber = PhoneNumberField("Номер владельца", max_length=20, db_index=True)
    owners_phonenumber_pure = PhoneNumberField("Нормализованный номер владельца", max_length=20, blank=True, db_index=True)
